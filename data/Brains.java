...
//			points.run(currentSubject, indivPoints, Color.BLACK);
        ...
        try (Writer brw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("smallworld.txt"), "utf-8"))) {
            try {
                brw.write("Subject ID : Small World\n");
                for (Map.Entry entry:swMeasures.entrySet()) {
                    brw.write(entry.getKey() + " ");
                    brw.write(":");
                    brw.write(" " + entry.getValue());
                    brw.write("\n");
                }
            } catch (IOException e) {
